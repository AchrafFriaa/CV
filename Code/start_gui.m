classdef start_gui < matlab.apps.AppBase
    ...
    % Calculate image gradients in x-direction
          ...
          function cost = matching_cost_ncc()
        % Normalize images
        % img1 = bsxfun(@rdivide,double(image_l),std(double(image_l)));
        % img2 = bsxfun(@rdivide,double(image_r),std(double(image_r)));
        % img1 = (image_l(:)-mean(image_l(:))) / std(image_l(:))
            [nrow2, ncol2, ndim2]=size(image_r);
            win_size = 5;
        % f=fspecial('average', window_length);
        % Normierung
        % Suchfenster Indizes: [... -1 0 1 ...]
    ...
    block_size = 2 * block_half + 1;
        ...
        template_matcher = cell(block_size);
        ...
        % Iterate over columns
            ...
            % in the other image
                ...
                % differences (SAD)
                    block_dis(block_index, 1) = sum(sum(abs(template - block)));
                end
            % Sort vector containing disparities & get index of first one
                [temp, sorted_dis] = sort(block_dis);
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.DisparitygeneratorUIFigure.Position = [100 100 632 561];
            ...
            app.LoadImageButton.Position = [271 330 100 22];
            ...
            app.UIAxes.Position = [13 365 300 185];
            ...
            app.UIAxes2.Position = [326 364 300 185];
            ...
            app.UIAxes3.Position = [165 57 297 210];
            ...
            app.GenerateDisparityMapButton.Position = [250 36 142 22];
